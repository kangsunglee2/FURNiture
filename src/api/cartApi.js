...
export const addToCart = async (cartItem) => {
  try {
    const response = await axios.post(`${server}/ft/api/v2/carts`, cartItem);
...
export const fetchCartItem = async (currentUserEmail) => {
  try {
    const response = await axios.get(`${server}/ft/api/v2/carts/list/${currentUserEmail}`);
    return response.data;
  } catch (error) {
...
export const deleteCartItem = async (currentUserEmail, cid) => {
  try {
    const response = await axios.delete(`${server}/ft/api/v2/carts/delete/${currentUserEmail}`, {
      data: [cid]
    });
    return response;
  } catch (error) {
...
export const deleteAllCartItems = async (currentUserEmail) => {
  try {
    const response = await axios.post(`${server}/ft/api/v2/carts/delete/${currentUserEmail}`);
    return response.data;
  } catch (error) {
    console.log('상품 삭제 실패:', error);
    throw error;
...
export const updateCartItemQuantity = async (cartId, currentUserEmail, itemId, itemOption, newQuantity) => {
  try {
    ...
    const response = await axios.post(`${server}/ft/api/v2/carts/update`, {
      cid: cartId,
